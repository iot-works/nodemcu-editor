...
var settings = {
    ...
    apiUrl: "https://api.iot-works.com",
    access_token: null,
    minimumApiDelay: 500,
    ...
    dirIncludeFilename: "iot.include",
    dirExcludeFilename: "iot.ignore",

    knownApps: {
        "nodemcu": "binaries/nodemcu.bin"
    },
...
settings.ensureFolder = function() {
    var iotDir = path.join(settings.findHomePath(), ".iot");
...
settings.findOverridesFile = function(profile) {
    profile = profile || settings.profile || "iot";

    var iotDir = settings.ensureFolder();
    return path.join(iotDir, profile + ".config.json");
};
...
settings.whichProfile = function() {
    settings.profile = "iot";

    var iotDir = settings.ensureFolder();
    var proFile = path.join(iotDir, "profile.json");      //proFile, get it?
    if (fs.existsSync(proFile)) {
        var data = JSON.parse(fs.readFileSync(proFile));

        settings.profile = (data) ? data.name : "iot";
